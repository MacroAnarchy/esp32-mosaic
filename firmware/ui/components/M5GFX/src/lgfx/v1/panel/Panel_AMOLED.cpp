...
*/
        ...
        void Panel_AMOLED_Framebuffer::display(uint_fast16_t x, uint_fast16_t y, uint_fast16_t w, uint_fast16_t h)
        {
            ...
            // Banded transfer (Mosaic node): batch several rows into one DMA
            // transaction. The ESP32 bus driver is synchronous per call, so a
            // per-line flush of a 466-row panel costs ~30ms/frame (task
            // watchdog starves the idle task). Batching 16 rows cuts the
            // transaction count ~30x and keeps the flush inside the 33fps
            // budget; the double-buffered copy keeps the band-overwrite race
            // impossible (each band is copied to its own buffer before the
            // synchronous write).
            constexpr size_t BAND_ROWS = 16;
            size_t band_bytes = wb * BAND_ROWS;
            uint8_t* buf[2];
            buf[0] = bus->getDMABuffer(band_bytes);
            buf[1] = bus->getDMABuffer(band_bytes);
            ...
            while (remaining > 0)
            {
                size_t rows = std::min(remaining, BAND_ROWS);
                auto lb = buf[i & 1];
                auto dst = lb;
                auto src = &_frame_buffer[fbpos];
                ...
                fbpos += stride * rows;
                bus->writeBytes(lb, wb * rows, false, true);
                remaining -= rows;
                i++;
            }
